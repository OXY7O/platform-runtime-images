...
docker run --rm \
  ...
  --workdir /workspace \
  "${image}" \
  sh -c 'test -f composer.lock && composer install --no-interaction --no-progress --prefer-dist --no-scripts && npm cache verify && vendor/bin/phpunit tests/RuntimeSmokeTest.php'
